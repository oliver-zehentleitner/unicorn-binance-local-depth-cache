...
async def main():
    ...
    while ubldc.is_stop_request() is False:
        ...
        for market in markets:
            try:
                top_asks_limit = ubldc.cluster.get_asks(
                    market=market, limit_count=limit_count
                )
                top_bids_limit = ubldc.cluster.get_bids(
                    market=market, limit_count=limit_count
                )
                top_asks_threshold = ubldc.cluster.get_asks(
                    market=market,
                    limit_count=threshold_volume_limit_count,
                    threshold_volume=threshold_volume,
                )
                top_bids_threshold = ubldc.cluster.get_bids(
                    market=market,
                    limit_count=threshold_volume_limit_count,
                    threshold_volume=threshold_volume,
                )
            ...
            depth = (
                f"depth_cache '{market}' is in sync: \r\n "
