...
async def main():
    ...
    for dcl_exchange in dc["depthcache_list"]:
        print(
            f"Stopping {len(dc['depthcache_list'][dcl_exchange])} DepthCaches for exchange '{dcl_exchange}' on UBDCC "
            f"'{ubdcc_address}'!"
        )
        loop = 1
        for market in dc["depthcache_list"][dcl_exchange]:
            ...
            pprint(
                ubldc.cluster.stop_depthcache(
                    exchange=dcl_exchange, market=market, debug=False
                )
            )
            loop += 1
