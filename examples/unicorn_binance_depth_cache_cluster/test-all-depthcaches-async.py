...
async def main():
    ...
    for dcl_exchange in dc["depthcache_list"]:
        print(
            f"Testing {len(dc['depthcache_list'][dcl_exchange])} DepthCaches for exchange '{dcl_exchange}' on UBDCC "
            f"'{ubdcc_address}'!"
        )
        loop = 1
        for market in dc["depthcache_list"][dcl_exchange]:
            asks = await ubldc.cluster.get_asks_async(
            ...
            if asks.get("error_id") is not None:
                print(
                    f"Asks from DepthCache #{loop} '{market}' failed: {asks.get('error_id')} - {asks.get('message')}"
                )
                pprint(asks)
