...
with BinanceLocalDepthCacheManager(warn_on_update=False) as ubldc:
    if ubldc.is_update_available():
        print(
            f"Please upgrade to {ubldc.get_latest_version()} you are on {ubldc.get_version()}"
        )
        latest_release_info = ubldc.get_latest_release_info()
        if latest_release_info:
            print(
                f"Please download the latest release or run `pip install unicorn-binance-local-depth-cache --upgrade`"
